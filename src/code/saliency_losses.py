...
def modified_kldiv(s_map, gt,disc_map):
 #   print(s_map.size())
 #   print(gt.size())
 #   print(disc_map.size())
    batch_size = s_map.size(0)
    ...
    assert expand_gt.size() == gt.size()
    sum_disc_map = torch.sum(disc_map.view(batch_size, -1), 1)
    expand_disc = sum_disc_map.view(batch_size, 1, 1).expand(batch_size, w, h)

    disc_map = disc_map/(expand_disc*1.0)
    disc_map = disc_map.view(batch_size, -1)
    s_map = s_map/(expand_s_map*1.0)
    gt = gt / (expand_gt*1.0)
    ...
    eps = 2.2204e-16
    result = disc_map * torch.log(eps + gt/(s_map + eps))
    # print(torch.log(eps + gt/(s_map + eps))   )
    return torch.mean(torch.sum(result, 1))
...
def nss(s_map, gt):
  #  print("SIZE BEFORE:",s_map.size(), gt.size())
    if s_map.size() != gt.size():
        ...
        s_map = s_map.cuda()
        gt = gt.cuda()
 #   print("SIZE AFTER:",s_map.size(), gt.size())
...
def auc_judd(saliencyMap, fixationMap, jitter=True, toPlot=False, normalize=False):
    ...
    saliencyMap = saliencyMap.cpu().squeeze(0).numpy()
    fixationMap = fixationMap.cpu().squeeze(0).numpy()
    if normalize:
