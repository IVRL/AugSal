...
def diffusion_step_atn_replaced(model, controller, latents, context, t, guidance_scale, low_resource=False):
...
#         print (p.requires_grad)
    if low_resource:
        ...
        noise_pred_uncond, noise_prediction_text = noise_pred.chunk(2)
    noise_pred = noise_pred_uncond + guidance_scale * (noise_prediction_text - noise_pred_uncond)
#     print("latents grad00 ", latents.requires_grad)

    latents = model.scheduler.step(noise_pred, t, latents)["prev_sample"]
#     print("latents grad01 ", latents.requires_grad)
...
#             if attn.shape==torch.Size([16, 256, 77]):
            if attn.shape[-1]== 77: #cross attention
...
#             print(to_out(out).shape)
    ...
    def my_register_recr(net_, count, place_in_unet):
        if net_.__class__.__name__ == 'CrossAttention':
            net_.forward = my_ca_forward(net_, place_in_unet)
            return count + 1
        elif hasattr(net_, 'children'):
            for net__ in net_.children():
                count = my_register_recr(net__, count, place_in_unet)
        return count
    ...
    for net in sub_nets:
        if "down" in net[0]:
            cross_att_count += my_register_recr(net[1], 0, "down")
        elif "up" in net[0]:
            cross_att_count += my_register_recr(net[1], 0, "up")
        elif "mid" in net[0]:
            cross_att_count += my_register_recr(net[1], 0, "mid")
